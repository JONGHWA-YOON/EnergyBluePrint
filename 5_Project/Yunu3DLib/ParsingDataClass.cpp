...
namespace ASE
{
    ...
    Mesh::~Mesh()
    {
        auto deleteAndClear = []<typename Container>(Container & container)
        {
        ...
        auto deleteAndSetNull = []<typename JsonStructType>(JsonStructType * &pointer)
        {
            delete pointer;
            pointer = nullptr;
        };

        deleteAndClear(m_meshvertex);
        deleteAndClear(m_meshface);		// 인덱스
        deleteAndClear(m_mesh_tvertex);		// 인덱스

        deleteAndClear(m_opt_vertex);		// 인덱스
        deleteAndSetNull(m_opt_index);

        deleteAndClear(m_vector_shape_line);		// 인덱스
        deleteAndClear(m_list_lightanimation);		// 인덱스


        deleteAndClear(m_vector_bone_list);		// 인덱스
        deleteAndClear(m_vector_wvertexs);		// 인덱스
    }
