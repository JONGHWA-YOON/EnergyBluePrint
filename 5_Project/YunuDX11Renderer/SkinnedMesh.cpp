...
void render_object::SkinnedMesh::Initialize()
{
    ...
    mTech = Effects::skinnedBasicTexFX->defaultTech.Get();
...
void render_object::SkinnedMesh::RenderToTexture()
{
    // 입력 배치 객체 셋팅
    md3dDeviceContext->IASetInputLayout(InputLayouts::SkinnedBasic);
    ...
    for (UINT p = 0; p < techDesc.Passes; ++p)
    {
        ...
        Effects::skinnedBasicTexFX->deferredGeometryTech->GetPassByIndex(0)->Apply(0, md3dDeviceContext);
        md3dDeviceContext->Draw(vertexCount, 0);

        DeferredBuffers::SingleInstance.Get().RenderTargetFrontBuffer(md3dDeviceContext);
        Effects::skinnedBasicTexFX->defaultTech->GetPassByIndex(0)->Apply(0, md3dDeviceContext);
        md3dDeviceContext->Draw(vertexCount, 0);
    }
}
