...
render_object::Crate::Crate(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, ID3D11RasterizerState* pRS)
...
void render_object::Crate::Initialize()
{
...
void render_object::Crate::Update(Camera* pCamera, float deltaTime)
{
...
void render_object::Crate::RenderToTexture()
{
    // 입력 배치 객체 셋팅
    md3dDeviceContext->IASetInputLayout(InputLayouts::Basic32);
    ...
    ID3DX11EffectTechnique* activeTech = Effects::BasicTexFX->Light2TexTech.Get();
    ...
    for (UINT p = 0; p < techDesc.Passes; ++p)
    {
        ...
        activeTech->GetPassByIndex(p)->Apply(0, md3dDeviceContext);
        md3dDeviceContext->DrawIndexed(mBoxIndexCount, mBoxIndexOffset, mBoxVertexOffset);
    }
}

void render_object::Crate::BuildGeometryBuffers()
{
