...
render_object::Helper::Grid::Grid(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, ID3D11RasterizerState* pRS)
...
render_object::Helper::Grid::~Grid()
{
    mFX->Release();
...
void render_object::Helper::Grid::Initialize()
{
...
void render_object::Helper::Grid::Update(const XMMATRIX& world, const XMMATRIX& view, const XMMATRIX& proj)
{
    mWorld = world;
    mView = view;
    mProj = proj;
...
void render_object::Helper::Grid::RenderToTexture()
{
    // 입력 배치 객체 셋팅
    ...
    for (UINT p = 0; p < techDesc.Passes; ++p)
    {
        ...
        // 20개의 인덱스로 그리드를 그린다.
        md3dDeviceContext->DrawIndexed(40, 0, 0);
    }
}

void render_object::Helper::Grid::BuildGeometryBuffers()
{
    // 정점 버퍼를 생성한다. 
    // 40개의 정점을 만들었다.
    Vertex vertices[100];
...
void render_object::Helper::Grid::BuildFX()
{
...
void render_object::Helper::Grid::BuildVertexLayout()
{
