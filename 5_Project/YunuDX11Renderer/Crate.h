...
namespace render_object
{
    ...
    public:
        void Initialize();	// 초기화
        void Update(Camera* pCamera, float deltaTime);		// 현재는 업데이트 시 변환행렬만 갱신한다.
        void RenderToTexture();		// 그린다. 엔진의 BeginRender()와 EndRender()사이에 호출 될 것을 기대한다.
